...
function custom_hg_prompt() {
    hg prompt --angle-brackets "$ZSH_THEME_GIT_PROMPT_PREFIX<branch>$ZSH_THEME_GIT_PROMPT_SUFFIX" 2>/dev/null
}

# %B sets bold text
PROMPT='%B$PREFIX %2~ $(custom_git_prompt)$(custom_hg_prompt)%{$M%}%B»%b%{$RESET%} '
RPS1="${return_code}"
